...
- [第三章：谁来真正读取文件？](docs/chapters/03-tool-execution.mdx)：受限文件读取、错误结果、调用配对与上下文回传。
- [第三章代码与运行说明](examples/ch03_tool_execution/README.md)：临时样例目录、真实本地读取和有边界的两次模型交接。
...
前三章内容与配套示例已编写，其余章节仍在设计中，网站尚未搭建。计划使用 Docusaurus 承载 Markdown/MDX 内容，按需加入 React 交互图解。源码案例与教学简化实现分别标注；离线测试与真实 API 验证分别记录。
