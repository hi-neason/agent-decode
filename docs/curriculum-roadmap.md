...
状态：v0.5，2026-09-22。面向程序员，中文优先。[第一章](chapters/01-llm-apis-and-streaming.mdx)已包含三类 API 与流式处理，[第二章](chapters/02-tool-call-requests.mdx)已实现工具请求识别；[第三章](chapters/03-tool-execution.mdx)已实现受限读取与结果回传；第四章及以后仍为设计草案，随案例研究调整。
