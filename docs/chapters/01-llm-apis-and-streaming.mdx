...
示例按已支持的顺序文本事件追加显示，不提供通用的多输出索引重排；Responses 最终文本还会与增量结果核对。需要并行工具、多种 block 或完整事件重建时，应保留 item／block 身份，下一节的 Pi 源码展示了这种做法。
...
## 7. 真实项目怎样处理：Pi 的三个 Provider

以下依据 Pi 固定提交 `71dca871bc80b6bc97be37f0ca3189399d651fff`，是源码定点分析，未运行该项目的集成测试。完整项目快照信息见 [源码案例调研](../source-study.md)。

### 7.1 Chat：先收 usage，再决定有没有正文

在 [`openai-completions.ts` 的流处理](https://github.com/earendil-works/pi/blob/71dca871bc80b6bc97be37f0ca3189399d651fff/packages/ai/src/api/openai-completions.ts#L558-L592)中，Pi 先处理 `chunk.usage`，再读取 `choices[0]`，最后追加 `choice.delta.content`。

这个顺序直接解决空 choices 用量块的问题。它还保存原始停止原因，再映射到自己的内部表示。注意这里消费的是 SDK 已解析的 chunk，不能拿这段代码声称 Pi 在这里自行解析了原始 `[DONE]`。

Pi 对部分兼容 Provider 有明确的结束原因例外，见[流结束检查](https://github.com/earendil-works/pi/blob/71dca871bc80b6bc97be37f0ca3189399d651fff/packages/ai/src/api/openai-completions.ts#L675-L695)。本章示例选择严格要求官方形状；两者是在支持范围上作了不同取舍。

### 7.2 Responses：结束快照用于校正，不用于重复追加

Pi 在 [`openai-responses-shared.ts`](https://github.com/earendil-works/pi/blob/71dca871bc80b6bc97be37f0ca3189399d651fff/packages/ai/src/api/openai-responses-shared.ts#L633-L709) 中处理文本／拒绝增量，并在输出项完成时根据内容快照更新文本。

它在[响应级终态处理](https://github.com/earendil-works/pi/blob/71dca871bc80b6bc97be37f0ca3189399d651fff/packages/ai/src/api/openai-responses-shared.ts#L741-L784)中区分 completed、incomplete 和失败，没有把 `response.output_text.done` 当成整个请求完成。

其中 token 限制导致的 incomplete 会映射为内部 `length`。这说明通用客户端的 `done` 事件仍需附带原因；上层 Agent 应决定是否继续、压缩上下文或报告限制，不能只看事件名称。

### 7.3 Messages：按 block index 保留类型和归属

Pi 在 [`anthropic-messages.ts`](https://github.com/earendil-works/pi/blob/71dca871bc80b6bc97be37f0ca3189399d651fff/packages/ai/src/api/anthropic-messages.ts#L621-L696) 中创建不同类型的 block，并根据事件索引定位增量所属的内容。

它在[用量更新](https://github.com/earendil-works/pi/blob/71dca871bc80b6bc97be37f0ca3189399d651fff/packages/ai/src/api/anthropic-messages.ts#L740-L775)时更新出现的字段，在[流结束检查](https://github.com/earendil-works/pi/blob/71dca871bc80b6bc97be37f0ca3189399d651fff/packages/ai/src/api/anthropic-messages.ts#L461-L499)时检查消息结束，避免把仅收到部分内容的连接视为成功。

三个实现共同保留的，不是“最终一段字符串”，而是**类型化内容、事件归属、用量和停止原因**。本章先展示文本这一部分；后续要驱动工具执行，就必须扩展这份结果结构。
